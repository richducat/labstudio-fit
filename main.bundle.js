(function initializeLabBundle() {
  const isTemplate = (value) => Boolean(value && value.__isTemplate);

  function html(strings, ...values) {
    return { __isTemplate: true, strings, values };
  }
  ...
  let currentHookState = null;
  let hookIndex = 0;
  let effectQueue = [];
  let rootElement = null;
  let rootTemplate = null;
  let componentPathStack = [0];

  const resetRenderState = () => {
    effectQueue = [];
    componentPathStack = [0];
  };

  const nextComponentPath = () => {
    const parentPath = componentPathStack.join('.');
    const idx = componentPathStack[componentPathStack.length - 1];
    componentPathStack[componentPathStack.length - 1] += 1;
    return `${parentPath}.${idx}`;
  };

  const normalizeChildren = (values) => {
    const nodes = [];
    values.forEach((value) => {
      ...
      if (Array.isArray(value)) {
        nodes.push(...normalizeChildren(value));
        return;
      }
      if (isTemplate(value)) {
        nodes.push(...renderTemplate(value).childNodes);
        return;
      }
  ...
  const isAttributePosition = (segment) => /=[\s"']*$/.test(segment);
  const isTagPosition = (segment) => /<\/*$/.test(segment);
  ...
  const renderTemplate = (template) => {
    ...
    strings.forEach((segment, index) => {
      ...
      if (isTagPosition(segment)) {
        markup += `x-htm-${index}`;
        markers[index] = { type: 'tag' };
      } else if (isAttributePosition(segment)) {
        markup += `__htm_attr_${index}__`;
        markers[index] = { type: 'attr' };
      } else {
        markup += `<!--htm_node_${index}-->`;
        markers[index] = { type: 'node' };
      }
    });
    ...
    while (commentNode) {
      const match = commentNode.data.match(/^htm_node_(\d+)$/);
      if (match) {
        const idx = Number(match[1]);
        const nodes = normalizeChildren([values[idx]]);
        const parent = commentNode.parentNode;
        nodes.forEach((node) => parent.insertBefore(node, commentNode));
    ...
    const componentElements = elements.filter((element) => element.tagName.toLowerCase().startsWith('x-htm-'));
    componentElements.forEach((element) => {
      const idx = Number(element.tagName.toLowerCase().replace('x-htm-', ''));
      const component = values[idx];
      if (typeof component !== 'function') {
        const replacement = document.createElement(component || 'div');
      ...
      Array.from(element.attributes).forEach((attr) => {
        const match = attr.value.match(/^__htm_attr_(\d+)__$/);
        const resolvedValue = match ? values[Number(match[1])] : attr.value;
        props[attr.name] = resolvedValue;
      });
      props.children = Array.from(element.childNodes);

      const path = nextComponentPath();
      const prevHookState = currentHookState;
      const prevHookIndex = hookIndex;
      currentHookState = hookStateMap.get(path) || [];
      hookStateMap.set(path, currentHookState);
      hookIndex = 0;
      componentPathStack.push(0);
      const rendered = component(props);
      componentPathStack.pop();
      currentHookState = prevHookState;
      hookIndex = prevHookIndex;

      const nodes = normalizeChildren([rendered]);
      element.replaceWith(...nodes);
    });

    const remainingElements = Array.from(fragment.querySelectorAll('*')).filter(
      (element) => !element.tagName.toLowerCase().startsWith('x-htm-')
    );
    remainingElements.forEach((element) => {
      Array.from(element.attributes).forEach((attribute) => {
        const match = attribute.value.match(/^__htm_attr_(\d+)__$/);
        if (!match) return;
        const idx = Number(match[1]);
        ...
        if (attribute.name === 'className') {
          if (value != null) element.setAttribute('class', String(value));
          return;
        }

        if (attribute.name === 'style' && value && typeof value === 'object') {
        ...
        if (attribute.name.startsWith('on') && typeof value === 'function') {
          element.addEventListener(attribute.name.slice(2).toLowerCase(), value);
          return;
        }
  ...
  const renderValue = (value) => {
    if (value === null || value === undefined || value === false) return document.createDocumentFragment();
    if (isTemplate(value)) return renderTemplate(value);
    if (Array.isArray(value)) {
      const fragment = document.createDocumentFragment();
      normalizeChildren(value).forEach((node) => fragment.appendChild(node));
      return fragment;
    }
    if (value instanceof Node) {
      const fragment = document.createDocumentFragment();
  ...
  const renderApp = () => {
    if (!rootElement || !rootTemplate) return;
    resetRenderState();
    rootElement.innerHTML = '';
    rootElement.appendChild(renderValue(rootTemplate));
    effectQueue.forEach((effect) => effect());
  };

  const useState = (initialValue) => {
    const idx = hookIndex;
    if (!currentHookState) throw new Error('useState called outside component');
    if (currentHookState[idx] === undefined) {
      currentHookState[idx] = typeof initialValue === 'function' ? initialValue() : initialValue;
    }
    const setState = (value) => {
      const nextValue = typeof value === 'function' ? value(currentHookState[idx]) : value;
      currentHookState[idx] = nextValue;
      renderApp();
    };
    hookIndex += 1;
  ...
  const useRef = (initialValue) => {
    const idx = hookIndex;
    if (!currentHookState) throw new Error('useRef called outside component');
    if (!currentHookState[idx]) {
      currentHookState[idx] = { current: initialValue };
    }
    hookIndex += 1;
    return currentHookState[idx];
  };

  const useEffect = (effect, deps) => {
    const idx = hookIndex;
    if (!currentHookState) throw new Error('useEffect called outside component');
    const prev = currentHookState[idx];
    let hasChanged = true;
    if (prev && deps) {
  ...
  const createIcon = (label) => ({ size = 24, className = '', strokeWidth = 2, fill = 'none' } = {}) => html`
    <svg
      aria-label=${label}
      viewBox="0 0 24 24"
      width=${size}
